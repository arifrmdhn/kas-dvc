...
const firebaseConfig = {
  apiKey: "AIzaSyBd85JZt-YQOZOiRn3YHS7-ir4tmX5rY88",
  authDomain: "kas-dvc.firebaseapp.com",
  projectId: "kas-dvc",
  storageBucket: "kas-dvc.firebasestorage.app",
  messagingSenderId: "363428218791",
  appId: "1:363428218791:web:c3520cdf945c4b81699882",
  measurementId: "G-NXNQ23LMWS"
};
...
export default function App() {
  ...
  useEffect(() => {
    // Listener untuk Anggota
    const qMembers = query(collection(db, 'members'), orderBy('createdAt', 'asc'));
  ...
  const formatShortRupiah = (number) => new Intl.NumberFormat('id-ID').format(number);
  ...
  // --- HANDLERS KE FIREBASE (TAMBAH DATA) ---
  ...
  // --- HANDLERS KE FIREBASE (EDIT & HAPUS DATA) ---
  ...
  const handleUpdateTx = async (e) => {
    ...
    await updateDoc(doc(db, 'transactions', editingTx.id), {
      amount: parseInt(editingTx.amount),
          ...
          <tbody>
            {members.map(member => (
              <tr key={member.id} className="hover:bg-indigo-50/30 transition-colors group">
                <td className="p-4 border-b border-r font-medium text-gray-800 sticky left-0 bg-white group-hover:bg-indigo-50/10 z-10 shadow-[1px_0_0_#f3f4f6]">
                  <div className="flex items-center justify-between gap-2">
                    <span>{member.name}</span>
                    <button onClick={() => openEditMember(member)} className="text-gray-400 hover:text-indigo-600 p-1 rounded-md hover:bg-indigo-50 transition-colors" title="Edit/Hapus Anggota">
                      <Edit2 size={14} />
                    </button>
                  </div>
                </td>
                    ...
                    <td key={p.id} className="p-2 border-b border-r text-center align-middle">
                      {tx ? (
                        <button 
                          ...
                          className="inline-block bg-emerald-100 text-emerald-700 text-xs font-bold px-2 py-1 rounded cursor-pointer hover:bg-emerald-200 transition-colors"
                          title={`Edit/Hapus: ${tx.description} - ${tx.date}`}
                        >
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      <div className="p-6 border-b border-gray-100 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500">Daftar seluruh riwayat pemasukan dan pengeluaran dari Database.</p>
        </div>
      </div>
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {transactions.map(tx => (
              <tr key={tx.id} className="hover:bg-gray-50 transition-colors text-sm">
                ...
                <td className="p-4 border-b text-center">
                  <button onClick={() => openEditTx(tx)} className="inline-flex items-center justify-center p-1.5 text-gray-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md transition-colors" title="Edit/Hapus Transaksi">
                    <Edit2 size={16} />
                  </button>
                </td>
              </tr>
            ...
            {transactions.length === 0 && (
              <tr>
                <td colSpan="6" className="p-8 text-center text-gray-500">Belum ada riwayat transaksi.</td>
              </tr>
            )}
          </tbody>
        </table>
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-900 selection:bg-indigo-100 selection:text-indigo-900">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            ...
            <nav className="hidden md:flex gap-1">
              <button onClick={() => setActiveTab('dashboard')} className={`px-4 py-2 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'dashboard' ? 'bg-slate-800 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-800/50'}`}>
                <Home size={18} /> Dashboard
              </button>
              <button onClick={() => setActiveTab('members')} className={`px-4 py-2 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'members' ? 'bg-slate-800 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-800/50'}`}>
                <Users size={18} /> Anggota & Kas
              </button>
              <button onClick={() => setActiveTab('transactions')} className={`px-4 py-2 rounded-md font-medium text-sm transition-colors flex items-center gap-2 ${activeTab === 'transactions' ? 'bg-slate-800 text-white' : 'text-slate-300 hover:text-white hover:bg-slate-800/50'}`}>
      ...
      <div className="md:hidden bg-white border-b border-gray-200 flex overflow-x-auto">
        <button onClick={() => setActiveTab('dashboard')} className={`flex-1 min-w-[120px] py-3 text-sm font-medium flex justify-center items-center gap-2 border-b-2 ${activeTab === 'dashboard' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>
          <Home size={16} /> Dashboard
        </button>
        <button onClick={() => setActiveTab('members')} className={`flex-1 min-w-[120px] py-3 text-sm font-medium flex justify-center items-center gap-2 border-b-2 ${activeTab === 'members' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>
          <Users size={16} /> Anggota
        </button>
        <button onClick={() => setActiveTab('transactions')} className={`flex-1 min-w-[120px] py-3 text-sm font-medium flex justify-center items-center gap-2 border-b-2 ${activeTab === 'transactions' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500'}`}>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-indigo-50">
              <h3 className="font-bold text-lg text-indigo-800">Edit Anggota</h3>
              <button onClick={() => setIsEditMemberModalOpen(false)} className="text-indigo-500 hover:text-indigo-700 text-xl font-bold">&times;</button>
            </div>
            <form onSubmit={handleUpdateMember} className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Bagian (Section)</label>
                <select value={editingMember.section} onChange={e => setEditingMember({...editingMember, section: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500">
                  <option value="Brass">Brass</option>
                  <option value="Percussion">Percussion (Battery/Pit)</option>
                  <option value="Color Guard">Color Guard</option>
                  <option value="Woodwind">Woodwind</option>
              ...
              <div className="pt-4 border-t border-gray-100 mt-6 flex justify-between items-center">
                <button type="button" onClick={() => handleDeleteMember(editingMember.id)} className="flex items-center gap-1 text-sm text-rose-600 hover:text-rose-800 font-medium px-2 py-2 rounded hover:bg-rose-50 transition-colors">
                  <Trash2 size={16} /> Hapus Anggota
                </button>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIsEditMemberModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50">Batal</button>
                  <button type="submit" className="px-4 py-2 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700">Simpan</button>
                </div>
              </div>
            </form>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleRecordPayment} className="p-6 space-y-4">
              ...
              <div>
                ...
                <input type="number" required min="0" value={paymentForm.amount} onChange={e => setPaymentForm({...paymentForm, amount: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500" placeholder="Contoh: 10000"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Keterangan / Catatan</label>
                <input type="text" required value={paymentForm.description} onChange={e => setPaymentForm({...paymentForm, description: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-emerald-500"/>
              </div>
              <div className="pt-4 flex gap-3">
      ...
      {/* Modal Edit / Hapus Transaksi (Kas & Pengeluaran) */}
      {isEditTxModalOpen && editingTx && (
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            <div className={`p-6 border-b border-gray-100 flex justify-between items-center ${editingTx.type === 'income' ? 'bg-emerald-50' : 'bg-rose-50'}`}>
              <h3 className={`font-bold text-lg ${editingTx.type === 'income' ? 'text-emerald-800' : 'text-rose-800'}`}>Edit Transaksi</h3>
              <button onClick={() => setIsEditTxModalOpen(false)} className={`${editingTx.type === 'income' ? 'text-emerald-500 hover:text-emerald-700' : 'text-rose-500 hover:text-rose-700'} text-xl font-bold`}>&times;</button>
            </div>
            <form onSubmit={handleUpdateTx} className="p-6 space-y-4">
              <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 shadow-sm mb-4">
                <p className="text-xs text-gray-500 uppercase tracking-wider font-semibold mb-1">Kategori Transaksi</p>
                <p className="font-bold text-gray-800 text-sm">
                  {editingTx.category} {editingTx.memberId ? `(${members.find(m => m.id === editingTx.memberId)?.name || 'Kas Anggota'})` : ''}
                </p>
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nominal (Rp)</label>
                <input type="number" required min="0" value={editingTx.amount} onChange={e => setEditingTx({...editingTx, amount: e.target.value})} className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 ${editingTx.type === 'income' ? 'focus:ring-emerald-500' : 'focus:ring-rose-500'}`} />
              </div>
              
              {editingTx.category !== 'Kas Anggota' && (
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Ubah Kategori</label>
                  <select value={editingTx.category} onChange={e => setEditingTx({...editingTx, category: e.target.value})} className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 ${editingTx.type === 'income' ? 'focus:ring-emerald-500' : 'focus:ring-rose-500'}`}>
              ...
              <div>
                ...
                <input type="text" required value={editingTx.description} onChange={e => setEditingTx({...editingTx, description: e.target.value})} className={`w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 ${editingTx.type === 'income' ? 'focus:ring-emerald-500' : 'focus:ring-rose-500'}`} />
              </div>
              
              <div className="pt-4 border-t border-gray-100 mt-6 flex justify-between items-center">
                <button type="button" onClick={() => handleDeleteTx(editingTx.id)} className="flex items-center gap-1 text-sm text-rose-600 hover:text-rose-800 font-medium px-2 py-2 rounded hover:bg-rose-50 transition-colors">
                  <Trash2 size={16} /> Hapus Transaksi
                </button>
                <div className="flex gap-2">
                  <button type="button" onClick={() => setIsEditTxModalOpen(false)} className="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 font-medium hover:bg-gray-50">Batal</button>
                  <button type="submit" className={`px-4 py-2 text-white rounded-lg font-medium ${editingTx.type === 'income' ? 'bg-emerald-600 hover:bg-emerald-700' : 'bg-rose-600 hover:bg-rose-700'}`}>Simpan</button>
                </div>
              </div>
            </form>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleRecordExpense} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nominal (Rp)</label>
                <input type="number" required min="0" value={expenseForm.amount} onChange={e => setExpenseForm({...expenseForm, amount: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500" placeholder="Contoh: 50000"/>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Kategori Pengeluaran</label>
                <select value={expenseForm.category} onChange={e => setExpenseForm({...expenseForm, category: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500">
                  <option value="Peralatan">Peralatan / Alat Musik</option>
                  <option value="Konsumsi">Konsumsi</option>
                  <option value="Transportasi">Transportasi</option>
                  <option value="Seragam">Seragam / Kostum</option>
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Keterangan / Rincian</label>
                <input type="text" required value={expenseForm.description} onChange={e => setExpenseForm({...expenseForm, description: e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-rose-500" placeholder="Contoh: Beli air mineral untuk latihan"/>
              </div>
              <div className="pt-4 flex gap-3">
